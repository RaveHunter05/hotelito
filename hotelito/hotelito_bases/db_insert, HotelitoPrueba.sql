...
INSERT INTO `hotelito_prueba`.`empleado`
(`cedula_em`,
`prinombre_emp`,
`segnombre_emp`,
`priapellido_emp`,
...
INSERT INTO `hotelito_prueba`.`usuario`
(`id_emp`,
`username_user`,
`password_user`,
`estado_user`)
...
INSERT INTO `hotelito_prueba`.`cliente`
(`cedula_cli`,
`prinombre_cli`,
`segnombre_cli`,
`priapellido_cli`,
...
INSERT INTO `hotelito_prueba`.`servicio`
(`nombre_ser`,
`tipo_ser`,
`descripcion_ser`,
`precio_ser`,
...
INSERT INTO `hotelito_prueba`.`mueble`
(`nombre_mue`,
`descripcion_mue`,
`precio_mue`,
`id_hab`)
...
INSERT INTO `hotelito_prueba`.`reservacion`
(`id_cli`,
`fecha_entrada`,
`fecha_salida`,
`cantidad_hab`,
...
INSERT INTO `hotelito_prueba`.`reservacion_habitaciones`
(`id_res`,
`id_hab`,
`subtotal`)
VALUES
...
(1,
2,
(select precio_hab from habitacion where id_hab=2));
...
INSERT INTO `hotelito_prueba`.`reservacion_huesped`
(`id_res`,
`id_cli`)
VALUES
(1,
...
INSERT INTO `hotelito_prueba`.`pago_reservacion`
(`id_res`,
`fecha`,
`forma_pago`)
VALUES
